...
private String suscriberfirstName;
    ...
    public void patientCoverage(java.io.PrintWriter output, String typeofCoverage, String suscriberId, int groupNumber, String claimAddress, String startDate, int suscriberRelation, String suscriberfirstName, String suscriberlastName, String suscriberDOB) {
        Scanner input = new Scanner(System.in);
        System.out.println("Visit coverage:  "+ typeofCoverage);
        // typeofCoverage = input.next();
        System.out.println(" Member Id number:  " +suscriberId);
        // suscriberId = input.next();
        System.out.println(" Group number:  " +groupNumber);
        // groupNumber = input.nextInt();
        System.out.println(" Claim Address:  " + claimAddress);
        // claimAddress = input.next();
        System.out.println("Start Date:  " + startDate);
        // startDate = input.next();
        System.out.println("What is patient relationship to the suscriber?     1-Self/2-Child/3-Spouse  " + suscriberRelation);
        // suscriberRelation = input.nextInt();
        if (suscriberRelation == 1) {
            System.out.println(" Patient is the subscriber"); }
         else {
            System.out.println(" Please enter subscribe first name: "+suscriberfirstName);
            // suscriberfirstName = input.next();
            System.out.println(" Please enter subscriber last name: " + suscriberlastName);
            // suscriberlastName = input.next();
            System.out.println(" Please enter subscriber Date of Birth " + suscriberDOB);
