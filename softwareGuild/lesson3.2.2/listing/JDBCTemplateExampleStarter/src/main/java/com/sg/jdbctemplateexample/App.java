...
public class App implements CommandLineRunner {
    ...
    private void displayList() throws SQLException {
        ...
        for (ToDo td : todos) {
            System.out.printf("%s: %s -- %s -- %s\n", td.getId(), td.getTodo(), td.getNote(), td.isFinished());
        }
        System.out.println("");
    }
    ...
    private void updateItem() throws SQLException {
        // Retrieve the object we are updating
        System.out.println("Update Item");
        System.out.println("Which item do you want to update?");
        String itemId = sc.nextLine();
        ToDo item = jdbc.queryForObject("SELECT * "
                                      + "FROM todo "
                                      + "WHERE id = ?", new ToDoMapper(), itemId);
        // Display the current item and ask what to update
        ...
        // Make the actual call to the database to update the row
        jdbc.update("UPDATE todo SET todo = ?, note = ?, finished = ? WHERE id = ?",
                item.getTodo(),item.getNote(),item.isFinished(),item.getId());
    ...
    // Create our mapper as an internal class to tie it directly to the class that is using it
